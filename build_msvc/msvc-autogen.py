#!/usr/bin/env python3
# Copyright (c) 2016-2022 The Testcoin Core developers
...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Testcoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/testcoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/testcoin_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='Testcoin-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/testcoin_config.h'), os.path.join(SOURCE_DIR, 'config/testcoin-config.h'))
