This directory contains the source code for the Testcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Testcoin UI.
...
- ClientModel is used by the main application `testcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a testcoin:... link from a browser).
...
* UI elements like TestcoinAmountField, which inherit from QWidget.
* `testcoinstrings.cpp`: automatically generated
* `testcoinunits.(h/cpp)`: TTC / mTTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/testcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `testcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `testcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `testcoin-qt`
9. You're all set! Start developing, building, and debugging the Testcoin Core GUI
