...
/** Call node.updateRwSetting() with Testcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Testcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/testcoin/testcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because testcoin 22.x will not try to read these.
...
//! Convert enabled/size values to testcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from testcoin -prune setting.
...
//! Get pruning size value to show in GUI from testcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayTestcoinUnit")) {
        settings.setValue("DisplayTestcoinUnit", QVariant::fromValue(TestcoinUnit::TTC));
    }
    QVariant unit = settings.value("DisplayTestcoinUnit");
    if (unit.canConvert<TestcoinUnit>()) {
        m_display_testcoin_unit = unit.value<TestcoinUnit>();
    } else {
        m_display_testcoin_unit = TestcoinUnit::TTC;
        settings.setValue("DisplayTestcoinUnit", QVariant::fromValue(m_display_testcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if testcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_testcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<TestcoinUnit>() == m_display_testcoin_unit) return;
    m_display_testcoin_unit = new_unit.value<TestcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayTestcoinUnit", QVariant::fromValue(m_display_testcoin_unit));
    Q_EMIT displayUnitChanged(m_display_testcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/testcoin-core/gui/issues/567).
