...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<TestcoinUnit>("TestcoinUnit");
#else
    qRegisterMetaType<TestcoinUnit>("TestcoinUnit");
#endif
}
...
static const char* qt_argv = "testcoin-qt";
...
void TestcoinApplication::setupPlatformStyle()
{
...
bool TestcoinApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new TestcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &TestcoinGUI::quitRequested, this, &TestcoinApplication::requestShutdown);
...
void TestcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool TestcoinApplication::baseInitialize()
{
...
void TestcoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &TestcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &TestcoinApplication::handleRunawayException);
    connect(this, &TestcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &TestcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void TestcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void TestcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TestcoinGUI::handlePaymentRequest);
            connect(window, &TestcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void TestcoinApplication::handleNonFatalException(const QString& message)
{
...
bool TestcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", TestcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see testcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see testcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse testcoin.conf, determine network, switch to network specific
