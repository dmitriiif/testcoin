#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Testcoin Core developers
...
TMPDIR_PREFIX = "testcoin_func_test_"
...
class TestcoinTestMetaClass(type):
    """Metaclass for TestcoinTestFramework.

    Ensures that any attempt to register a subclass of `TestcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'TestcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("TestcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("TestcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class TestcoinTestFramework(metaclass=TestcoinTestMetaClass):
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "testcoind": ("testcoind", "BITCOIND"),
            "testcoin-cli": ("testcoincli", "BITCOINCLI"),
            "testcoin-util": ("testcoinutil", "BITCOINUTIL"),
            "testcoin-wallet": ("testcoinwallet", "BITCOINWALLET"),
        }
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'testcoind', self.options.testcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'testcoin-cli', self.options.testcoincli) for v in versions]
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the testcoind
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    testcoind=self.options.testcoind,
                    testcoin_cli=self.options.testcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in testcoin.conf
    ...
    def skip_if_no_testcoind_tracepoints(self):
        """Skip the running test if testcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("testcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_testcoind_zmq(self):
        """Skip the running test if testcoind has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if testcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("testcoin-wallet has not been compiled")

    def skip_if_no_testcoin_util(self):
        """Skip the running test if testcoin-util has not been compiled."""
        if not self.is_testcoin_util_compiled():
            raise SkipTest("testcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if testcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("testcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether testcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether testcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_testcoin_util_compiled(self):
        """Checks whether testcoin-util was compiled."""
