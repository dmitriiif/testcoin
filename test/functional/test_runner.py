...
BASE_SCRIPTS = [
    ...
    'interface_testcoin_cli.py --legacy-wallet',
    'interface_testcoin_cli.py --descriptors',
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if testcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "testcoind"], stdout=subprocess.DEVNULL).returncode == 0:
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Testcoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/testcoin/testcoin/pull/27561.
