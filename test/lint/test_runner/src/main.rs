// Copyright (c) The Testcoin Core developers
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./src/config/testcoin-config.h.in";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args([
                    // These are exceptions which don't use testcoin-config.h, rather the Makefile.am adds
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <config/testcoin-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
^^^
One or more files use a symbol declared in the testcoin-config.h header. However, they are not
...
even though testcoin-config.h indicates that a faster feature is available and should be used.
...
#include <config/testcoin-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
^^^
None of the files use a symbol declared in the testcoin-config.h header. However, they are including
