# PSBT Howto for Testcoin Core

Since Testcoin Core 0.17, an RPC interface exists for Partially Signed Testcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/testcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Testcoin transactions that are not fully signed
...
#### Multisig with multiple Testcoin Core instances
...
Testcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `testcoin-cli` is used.
