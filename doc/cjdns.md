# CJDNS support in Testcoin Core

It is possible to run Testcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Testcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Testcoin network and individual nodes.
...
before it will work with your Testcoin Core node. This is described in steps
...
## Run Testcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Testcoin Core
...
When enabled, this option tells Testcoin Core that it is running in an
...
IPv6 local network. This helps Testcoin Core perform better address management:
...
CJDNS support was added to Testcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `testcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/testcoin/testcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Testcoin Core

There are several ways to see your CJDNS address in Testcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `testcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `testcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `testcoin-cli help getnodeaddresses` for details.
