...
Testcoin Core version 0.20.0 is now available from:

  <https://testcoincore.org/bin/testcoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Testcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Testcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Testcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Testcoin Core. Testcoin Core logs debug messages
...
- OpenSSL is no longer used by Testcoin Core.  (#17265)

- BIP70 support has been fully removed from Testcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/testcoin/testcoin/issues/16599),
  [PR #16702](https://github.com/testcoin/testcoin/pull/16702), and the
  `testcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Testcoin Core starts are now
...
- The "Start Testcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://testcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Testcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Testcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/testcoin-core/HWI) for
  signing.  Future versions of Testcoin Core should support a GUI option
...
- Testcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.testcoincore.org.  (#17596)
...
- The `testcoin-cli` utility used with the `-getinfo` parameter now
...
- The `testcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `testcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_testcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_testcoin` binary (hebasto)
...
- #18702 Fix ASLR for testcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate testcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest testcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_testcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (testcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone testcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Testcoin Core files (hebasto)
...
- #17119 Fix broken testcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from testcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.testcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to testcoin-wallet (hel-o)
...
- #16115 On testcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from testcoin-cli -getinfo (malevolent)
