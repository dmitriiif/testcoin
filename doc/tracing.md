# User-space, Statically Defined Tracing (USDT) for Testcoin Core

Testcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Testcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Testcoin Core
...
Multiple tools can list the available tracepoints in a `testcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Testcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/testcoind
stap net        outbound_message 0x0000000000107c05 /src/testcoind
stap validation block_connected  0x00000000002fb10c /src/testcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Testcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/testcoind | grep NT_STAPSDT -A 4 -B 2
