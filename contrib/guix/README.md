# Bootstrappable Testcoin Core Builds

This directory contains the files necessary to perform bootstrappable Testcoin
Core builds.
...
To build Testcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Testcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/testcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/testcoin-core/guix.sigs).
