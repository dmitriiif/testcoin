...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing testcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='testcoin-cli', help='testcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Testcoin address to which the faucet should send')
...
parser.add_argument('testcoin_cli_args', nargs='*', help='Arguments to pass on to testcoin-cli (default: -signet)')

args = parser.parse_args()

if args.testcoin_cli_args == []:
    args.testcoin_cli_args = ['-signet']


def testcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.testcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = testcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = testcoin_cli(['getnewaddress', 'faucet', 'bech32'])
