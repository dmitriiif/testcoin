#!/usr/bin/env python3
# Copyright (c) 2022 The Testcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Example logging Testcoin Core mempool events using the mempool:added,
...
def main(testcoind_path):
    testcoind_with_usdts = USDT(path=str(testcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    testcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    testcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    testcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    testcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[testcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/testcoind")
        exit(1)
