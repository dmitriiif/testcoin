...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$testcoin_qt_want_version" = "auto" && test "$testcoin_qt_force" != "yes"; then
    if test "$testcoin_enable_qt" != "no"; then
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$testcoin_enable_qt" != "no" && test "$testcoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build testcoin-qt GUI (default=auto)])],
    [
     testcoin_qt_want_version=$withval
     if test "$testcoin_qt_want_version" = "yes"; then
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); testcoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$testcoin_cv_static_qt" = "yes"; then
  ...
  if test $testcoin_enable_qt = "yes"; then
    AC_MSG_RESULT([$testcoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$testcoin_enable_qt])
  fi
